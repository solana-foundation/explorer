...
describe('extractProgramNameFromRepo', () => {
    it('extracts name from GitHub URL', () => {
        expect(extractProgramNameFromRepo('https://github.com/Ellipsis-Labs/phoenix-v1'))
            .toBe('Phoenix V1');
    });

    it('handles underscores', () => {
        expect(extractProgramNameFromRepo('https://github.com/org/token_metadata'))
            .toBe('Token Metadata');
    });
...
describe('getProgramName', () => {
    ...
    it('extracts repo name correctly', () => {
        expect(getProgramName(
            'unknown123',
            'https://github.com/org/my-program'
        )).toBe('My Program');
    });
