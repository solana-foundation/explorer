// Callers must pass a unique `id` when rendering multiple logos on the same page
// to avoid SVG ID collisions. We can't use `useId()` because next/og (Satori) doesn't support hooks.
type LogoProps = {
    id?: string;
...
export const Logo = ({ id = 'solana-logo', style, variant = 'gradient' }: LogoProps) => {
    const maskId = `${id}-mask`;
    const gradientId = `${id}-gradient`;
    const clipId = `${id}-clip`;
    const iconFill = variant === 'green' ? '#14F195' : `url(#${gradientId})`;
    ...
    return (
        ...
        >
            ...
            <g mask={`url(#${maskId})`}>
                ...
                />
                <g clipPath={`url(#${clipId})`} transform="scale(0.4375)">
                    <path
