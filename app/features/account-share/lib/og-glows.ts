...
/* The glow png background, one per account state.
 *
...
/** What {@link loadOgGlows} hands back, one base64 PNG data URI per account state. */
export type OgGlows = {
    /** The amber glow an absent account takes. */
    notFound: string;
    /** The green glow every resolved account and program takes. */
    success: string;
};

const FILES = {
    notFound: 'amber_gradient',
...
async function readGlowFiles(): Promise<OgGlows> {
    const [notFound, success] = await Promise.all([readGlowFile(FILES.notFound), readGlowFile(FILES.success)]);
