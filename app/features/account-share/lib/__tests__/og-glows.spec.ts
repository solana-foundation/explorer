...
describe('og-glows images', () => {
    it('should hand back a base64 PNG data URI per state', async () => {
        const glows = await loadOgGlows();

        expect(glows.success).toMatch(PNG_DATA_URI);
        expect(glows.notFound).toMatch(PNG_DATA_URI);
    });

    it('should draw the two states from different files', async () => {
        const glows = await loadOgGlows();

        expect(glows.success).not.toBe(glows.notFound);
    });

    it('should read once and hand back the same instance on a second call', async () => {
