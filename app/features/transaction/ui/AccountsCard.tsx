...
export function AccountsCard({ signature }: SignatureProps) {
    ...
    const accountRows = message.accountKeys.map((account, index) => {
        ...
        const badges = (
            <>
                {index === 0 && (
                    <Badge ui="dashkit" variant="success" className="e-me-1">
                        Fee Payer
                    </Badge>
                )}
                {account.signer && (
                    <Badge ui="dashkit" variant="info" className="e-me-1">
                        Signer
                    </Badge>
                )}
                {account.writable && (
                    <Badge ui="dashkit" variant="danger" className="e-me-1">
                        Writable
                    </Badge>
                )}
                {message.instructions.find(ix => ix.programId.equals(pubkey)) && (
                    <Badge ui="dashkit" variant="warning" className="e-me-1">
                        Program
                    </Badge>
                )}
                {account.source === 'lookupTable' && (
                    <Badge ui="dashkit" variant="gray" className="e-me-1">
