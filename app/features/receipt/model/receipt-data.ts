...
/**
 * Shaping a receipt out of a transaction the caller already holds.
 *
 * Universal on purpose: the receipt page runs this in the browser against the transaction its provider
 * already fetched. Anything that resolves a cluster endpoint belongs in `create-receipt.ts` instead,
 * which is server-only - a bundler follows modules rather than the exports you name, so mixing the two
 * here would pull the server cluster probe into the client bundle.
 */
