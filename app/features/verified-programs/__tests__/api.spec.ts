...
describe('fetchProgramsPage', () => {
    ...
    describe('error handling', () => {
        ...
        it('logs error when list API fails', async () => {
            ...
            expect(Logger.error).toHaveBeenCalledWith(expect.objectContaining({ message: 'Failed to fetch programs page 1' }));
        });
        ...
        it('logs error on network failure', async () => {
            ...
            expect(Logger.error).toHaveBeenCalledWith(expect.objectContaining({ cause: networkError, message: 'Failed to fetch programs page 1' }));
        });
        ...
        it('logs error for metadata fetch exception', async () => {
            ...
            expect(Logger.error).toHaveBeenCalledWith(expect.objectContaining({ cause: fetchError, message: `Failed to fetch metadata for ${mockProgramIds[0]}` }));
        });
    });
