...
describe('useAlpenglowStatus', () => {
    ...
    it('should resume polling a pending cluster after a remount', async () => {
        answerWith({ kind: 'absent' });
        const view = renderStatus({ dedupingInterval: 0 });
    ...
    it('should not report state on an unmounted component', async () => {
        const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {});
        mocks.fetchAgGenesisCert.mockReturnValue(new Promise(() => {}));

        renderStatus().unmount();
    ...
    it.each([
        ...
        ['the node answered with something that is not a certificate', { kind: 'unreadable' } as const],
    ])('should stop asking once %s', async (_label, answer) => {
        answerWith(answer);
