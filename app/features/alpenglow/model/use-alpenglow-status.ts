...
export function useAlpenglowStatus(): AlpenglowStatus {
    ...
    // A focus, a reconnect or a revisit never re-asks: for two of the three answers the next
    // request cannot say anything new, and the third has its own poll below. A settled endpoint
    // therefore costs exactly one request per cluster per session.
    //
    ...
    const { data, error } = useSWR(
        ...
        ([, , endpoint]: readonly ['alpenglow-genesis-cert', Cluster, ConnectableUrl]) => fetchAgGenesisCert(endpoint),
        {
            ...
            // `swr/immutable` is the shorter way to say the three below, but it also pins
            // `refreshInterval` to 0, which would take the poll above with it.
