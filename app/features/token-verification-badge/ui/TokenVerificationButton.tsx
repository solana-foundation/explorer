import { RISK_MAX_LEVEL_WARNING, RISK_MAX_LEVEL_GOOD } from '@/app/utils/rugcheck';
...
export function TokenVerificationButton({
    ...
    return (
        ...
        >
            <div className="mb-2 e-flex e-w-full e-items-center e-gap-2">
                ...
                <div className="e-flex e-items-center e-gap-1">
                    {hasVerification ? (
                        verificationFoundSources.map((source, idx) => (
                            <div
                                key={idx}
                                className={`e-flex e-rounded e-border e-border-solid e-p-px e-opacity-80 ${getSourceBorderColor(source)}`}
                            >
                                {source.icon}
                            </div>
                        ))
