...
export function resolveInstructionNames(ix: ParsedInstruction | PartiallyDecodedInstruction): InstructionNames {
    if (!('parsed' in ix)) {
        // Uint8Array.from normalizes the bs58 Buffer to a plain Uint8Array.
        return resolveNamesFromData({ data: Uint8Array.from(bs58.decode(ix.data)), programId: ix.programId });
    }
