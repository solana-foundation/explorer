...
import { getProgramMetadataIdl } from '@/app/entities/program-metadata/api/getProgramCanonicalMetadata';
import { getProgramNameFromIdl } from '@/app/entities/program-metadata/lib/getProgramNameFromIdl';
...
async function getPmName(address: string): Promise<string> {
    ...
    try {
        ...
        return getProgramNameFromIdl(idl) ?? '';
    } catch (error) {
        return '';
    }
}
