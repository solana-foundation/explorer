...
- **`app/entities/feature-gate/` is introduced** as the canonical home for feature-gate domain data:
  ...
  - `lib/feature-gates-schema.ts` defines `FeatureGateSchema` (and `FeatureGatesArraySchema`) in `superstruct`; the inferred `FeatureGate` type is the one type the writer (`scripts/update-feature-gates.ts`) and every reader import.
...
- **Feature Gates page UI is rewritten** under `app/features/feature-gate/ui/` (`FeatureGatesView`, `ActivatedFeaturesTable`, `UpcomingFeaturesTable`, `ExpandInfoButton`, `SimdLinks`). The page splits into **Activated** / **Upcoming** tabs (driven by the new `lib/partition-features.ts`), uses the shadcn `table` primitive (new at `app/components/shared/ui/table.tsx`), surfaces a per-row epoch countdown computed by `lib/epoch-countdown.ts`, and shows other-cluster activation status for upcoming features.
