...
- The proxy route `app/api/metadata/proxy/route.ts` also carries its own private `parseUrl` (alongside an `isHTTPProtocol` scheme check imported from the proxy `feature/`) — an in-route duplicate of the parse-and-check-scheme logic now centralised in `app/shared/lib/url.ts`. The route is already safe (it rejects non-http(s)), so this is a de-duplication tidy-up rather than a fix.
