...
- **A transport failure is never a verdict about the bytes** — `@solana/idl` reads every non-`SolanaError` throw out of an account read as `corrupt`, so the rpc wrapper that threads the abort signal also tags what the transport rejects with, and that tag rides back out as `IDL_FETCH_FAILED`: only bytes reach `IDL_PARSE_FAILED`, and an unreachable RPC stays retryable rather than being reported as a permanently invalid IDL. Known cost: a url-hosted PMP payload goes through upstream's own global `fetch`, untagged and unbounded, so that one blip still surfaces as a parse failure.
