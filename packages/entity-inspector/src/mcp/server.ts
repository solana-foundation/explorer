...
const buildInspectEntityDescription = (clusterNames: EnabledClusterNames) =>
    [
        ...
        '- "bpf-loader" / "bpf-loader-2" / "loader-v4": Legacy-loader programs — address, label, balance, executable flag and owner loader, plus the same "idl", "verification", "security_metadata" and "multisig" enrichments. These loaders have no upgrade authority and no separate programdata account, so "multisig" reports not_multisig and verification follows the frozen path; unavailable sources carry explicit unknown markers.',
