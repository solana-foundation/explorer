...
const buildInspectEntityDescription = (clusterNames: EnabledClusterNames) =>
    [
        ...
        '- "bpf-loader" / "bpf-loader-2" / "loader-v4": Legacy-loader programs — address, label, balance, executable flag and owner loader, plus the same "idl", "verification", "security_metadata" and "multisig" enrichments. None of the three has a separate programdata account. bpf-loader and bpf-loader-2 programs are immutable once finalized (executable true); loader-v4 keeps its authority and status inside account state this tool does not decode, so no upgrade authority is resolved for any of the three — "multisig" reports not_multisig and verification follows the frozen path. For loader-v4 an absent upgrade authority is a limit of this tool, not evidence the program is frozen. Unavailable sources carry explicit unknown markers.',
