...
Program decoders shared between the app slices and `@explorer/entity-inspector` live under
`src/programs/<program>` (exported as `@explorer/parsers/programs/<program>`), starting with the
System program.
...
The agadoo tree-shaking gate covers the root contract entry only: the `/compat` and
`/programs/*` entries define superstruct schemas at module scope, which Rollup cannot prove pure
without annotating every nested field call — and schema modules are imported for their struct
values wholesale, so the check adds nothing there (same trade-off as `@explorer/decoder-serum`).
